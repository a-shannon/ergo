...
class CandidateGeneratorPropSpec extends ErgoCorePropertyTest {
  ...
  property("a transaction exceeding candidate size should not starve later transactions") {
    ...
    val smallFeeTx = CandidateGenerator
      ...
      .get
    val maxSize = BlockTransactions.sizeOf(Seq(smallTx, smallFeeTx), h.version)

    BlockTransactions.sizeOf(Seq(largeTx, largeFeeTx), h.version) should be > maxSize
    ...
    BlockTransactions.sizeOf(selected, h.version) should be <= maxSize
    eliminated shouldBe empty
  ...
  property("descendants of a capacity-deferred transaction should remain in the mempool") {
    ...
    val independentFeeTx = CandidateGenerator
      ...
      .get
    val maxSize = BlockTransactions.sizeOf(Seq(accepted, independent, independentFeeTx), h.version)

    BlockTransactions.sizeOf(Seq(accepted, parent), h.version) should be > maxSize
