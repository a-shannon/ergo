...
import org.ergoplatform.network.ErgoNodeViewSynchronizerMessages.{ChangedMempool, CurrentWalletView, RequestCurrentWalletView}
...
class WalletProjectionRetentionSpec extends AnyPropSpec with Matchers with MempoolTestHelpers {
  ...
  private lazy val contextHeader = genChain(1).head.header.copy(height = height)
  ...
  private class ControlledStore(db: DB) extends LDBKVStore(db) {
    ...
    override def getRange(start: Array[Byte], end: Array[Byte], limit: Int): Array[(Array[Byte], Array[Byte])] =
      if (failRange) throw error else super.getRange(start, end, limit)
  }
  ...
  private class Service(config: ErgoSettings, val records: WalletStorage, seed: Seq[(ErgoTransaction, Int)])
    ...
    val loaded = new AtomicReference[ErgoWalletState]()
    @volatile var finalUpdate: ErgoWalletState => ErgoWalletState = identity
    override def updateUtxoState(state: ErgoWalletState): ErgoWalletState = finalUpdate(super.updateUtxoState(state))
    override def readWallet(state: ErgoWalletState, mnemonic: Option[SecretString], keys: Option[Int],
      ...
      records.updateStateContext(new ErgoStateContext(Seq(contextHeader), None, original.genesisStateDigest,
      ...
      state.registry.updateOnBlock(ScanResults(Seq(walletBox), Seq.empty, Seq.empty), contextHeader.id, height).get
      state.storage.close()
      val prepared = state.copy(storage = records, walletVars = WalletVars(Some(defaultProver), Seq(external), None)(config))
      loaded.set(prepared)
      prepared
    }
  ...
  private def withWallet(seed: Seq[(ErgoTransaction, Int)] = Seq.empty)(test: Fixture => Unit): Unit = {
    ...
    val actor = system.actorOf(Props(new ErgoWalletActor(config, parameters, service, null, null) {
      ...
      override protected[wallet] def readBestHeaderIdAtHeight(h: Int): Try[Option[ModifierId]] =
        Success(if (h == height) Some(contextHeader.id) else None)
    }))
    probe.watch(actor)
    try {
      val request = startup.expectMsgType[RequestCurrentWalletView](10.seconds)
      probe.send(actor, GetWalletStatus)
      probe.expectMsgType[WalletStatus](10.seconds)
      val context = service.loaded.get().stateContext
      val reader = Proxy.newProxyInstance(classOf[ErgoStateReader].getClassLoader, Array(classOf[ErgoStateReader]),
        new InvocationHandler {
          override def invoke(proxy: Any, method: Method, args: Array[AnyRef]): AnyRef = method.getName match {
            case "stateContext" => context
            case "toString" => "projection fixture state reader"
            case name => throw new IllegalStateException(s"Unexpected state reader call: $name")
          }
        }).asInstanceOf[ErgoStateReader]
      probe.send(actor, CurrentWalletView(request.requestId, reader, new FakeMempool(Seq.empty), None))
      probe.awaitAssert({
        probe.send(actor, GetWalletStatus)
        probe.expectMsgType[WalletStatus].error shouldBe None
      }, 10.seconds, 50.millis)
      val fixture = new Fixture(probe, actor, service, store)
      fixture.state
      test(fixture)
    }
    finally {
  ...
  property("persisted inputs stay reserved with no reader or an empty mempool without phantom outputs") {
    withWallet() { f =>
      val before = f.state
      before.copy(mempoolReaderOpt = None).walletFilter(walletBox) shouldBe true
      ...
      val noReader = stored.copy(mempoolReaderOpt = None)
      noReader.walletFilter(walletBox) shouldBe false
      f.absentView(noReader)
      f.absentView(stored)
  ...
  property("reconciliation honors an explicit dust limit including None instead of the settings fallback") {
    withWallet() { f =>
      val original = f.state
      val highDustSettings = original.walletVars.settings.copy(walletSettings =
        original.walletVars.settings.walletSettings.copy(dustLimit = Some(spending.outputs.head.value + 1)))
      val candidate = original.copy(walletVars = original.walletVars.copy()(highDustSettings),
        mempoolReaderOpt = Some(new FakeMempool(Seq(UnconfirmedTransaction(spending, None)))),
        offChainDustLimitOverride = None)
      candidate.offChainBoxes shouldBe empty
      val noDust = f.service.reconcileOffChainRegistry(candidate, None)
      noDust.offChainBoxes.map(_.boxId) shouldBe Seq(bytesToId(spending.outputs.head.id))
      noDust.walletVars.settings shouldBe highDustSettings
      val filtered = f.service.reconcileOffChainRegistry(noDust, Some(spending.outputs.head.value + 1))
      filtered.offChainBoxes shouldBe empty
      filtered.persistedInputIds shouldBe empty
    }
  }

  property("a final copied reader failure keeps the operational guard closed until a successful retry") {
    withWallet() { f =>
      val error = new IllegalStateException("final copied mempool read failed")
      val failing = new FakeMempool(Seq.empty) {
        override def getAllPrioritized: Seq[UnconfirmedTransaction] = throw error
      }
      f.service.finalUpdate = state => state.copy(mempoolReaderOpt = Some(failing))
      f.probe.send(f.actor, ChangedMempool(new FakeMempool(Seq.empty)))
      f.probe.send(f.actor, GetWalletStatus)
      f.probe.expectMsgType[WalletStatus].error.get should include(error.getMessage)
      f.probe.send(f.actor, GetWalletBoxes(true, true))
      f.probe.expectMsgType[akka.actor.Status.Failure]
      f.service.finalUpdate = identity
      val recovered = f.send(ChangedMempool(new FakeMempool(Seq.empty)))
      f.absentView(recovered)
    }
  }

  property("a final copied reservation read failure keeps records and the guard closed until retry") {
    withWallet() { f =>
      f.send(ScanOffChain(spending)).walletFilter(walletBox) shouldBe false
      f.service.finalUpdate = state => { f.store.failRange = true; state.copy() }
      f.probe.send(f.actor, ChangedMempool(new FakeMempool(Seq.empty)))
      f.probe.send(f.actor, GetWalletStatus)
      f.probe.expectMsgType[WalletStatus].error.get should include(f.store.error.getMessage)
      f.probe.send(f.actor, GetWalletBoxes(true, true))
      f.probe.expectMsgType[akka.actor.Status.Failure]
      f.store.failRange = false
      f.service.finalUpdate = identity
      val recovered = f.send(ChangedMempool(new FakeMempool(Seq.empty)))
      recovered.storage.readUnconfirmedTransactions().map(_._1.id) shouldBe Seq(spending.id)
      recovered.walletFilter(walletBox) shouldBe false
      f.absentView(recovered)
    }
  }
}
