...
case class ErgoWalletState(
    ...
    offChainDustLimitOverride: Option[Option[Long]] = None
  ...
  lazy val rawOffChainBoxes: Seq[TrackedBox] = mempoolReaderOpt match {
    case Some(mr) =>
      val dustLimit = offChainDustLimitOverride.getOrElse(walletVars.settings.walletSettings.dustLimit)
  ...
  /** Evaluate the final state's derived readers before a recovery boundary exposes it. */
  def materializeOffChainState(): ErgoWalletState = {
    val _ = (mempoolSpentIds, rawOffChainBoxes, offChainBoxes, offChainDigest, persistedInputIds)
    this
  }
